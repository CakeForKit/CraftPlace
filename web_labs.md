...
8. (done) Настроить /admin на проксирование в web-админку базы данных (можно подключить любую стандартную, внутреннюю или внешнюю, при этом она должна сразу, после авторизации, открывать доступ к БД приложения).
9. (done) Настроить /status на отдачу страницы статуса сервера. (Например, для  Nginx - nginx status).
10. (done) Сделать страничку /managment, где используя простейшую HTML-верстку разместить ссылки на страницы из п. 2/3,4,5,6,7,8,9.
